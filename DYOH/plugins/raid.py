...
for ubot in ubots:
    @ubot.on(events.NewMessage(pattern=f"{UBOT_HNDLR}raid (\\d+)(?: (\\d+))?"))
    async def raid(event):
        user_id = event.sender_id
        logging.info(f"Raid command received from user_id: {user_id}")
        ...
        target_user_id = event.pattern_match.group(2)

        if not event.reply_to_msg_id and not target_user_id:
        ...
        if target_user_id:
            try:
                ...
                user = await event.client.get_entity(target_user_id)
            except Exception:
        ...
        for i in range(10, 0, -1):
            ...
            await countdown_message.edit(f"{i} seconds remaining...")
    ...
    @ubot.on(events.NewMessage(pattern=f"{UBOT_HNDLR}replyraid$"))
    async def add_reply_raid(event):
        user_id = event.sender_id
        logging.info(f"Add reply raid command received from user_id: {user_id}")
    ...
    @ubot.on(events.NewMessage(pattern=f"{UBOT_HNDLR}dreplyraid$"))
    async def remove_reply_raid(event):
        user_id = event.sender_id
        logging.info(f"Remove reply raid command received from user_id: {user_id}")
